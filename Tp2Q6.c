...
void countingSort(Veiculo* carros, int expoente, int n){
	...
	for (int i = 0; i < n; i++) {
		int digito = (carros[i].ano / expoente) % 10;
    		count[digito]++;
	}
	...
	for (int i = n - 1; i >= 0; i--) {
		int digito = (carros[i].ano / expoente) % 10;
    		ordenado[count[digito] - 1] = carros[i];
    		count[digito]--;
	}
...
void radix(Veiculo* carros, int n){

	char res[2048];
	Veiculo* ordenado = (Veiculo*) malloc (n * sizeof (Veiculo));
	int maior = carros[0].ano;
	...
	for (int expoente = 1; maior / expoente > 0; expoente *= 10){
		countingSort (carros, expoente, n);
	}  	
